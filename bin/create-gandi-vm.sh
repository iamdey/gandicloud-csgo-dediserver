...
echo ""
echo "  Openstack application creation"
echo ""

# create the application credential to manage the server
APPLICATION_CREDENTIALS=$(openstack application credential create csgoserver)
APPLICATION_SECRET=$(echo "${APPLICATION_CREDENTIALS}" | grep -owP 'secret.*\|\s\K.*[^\|]+')
APPLICATION_ID=$(echo "${APPLICATION_CREDENTIALS}" | grep -owP 'id.*\|\s\K.*[^\|]+')
echo "# this file is automatically generated" > config/openstack.env
echo "OS_AUTH_URL=https://keystone.sd6.api.gandi.net:5000/v3" >> config/openstack.env
echo "OS_IDENTITY_API_VERSION=3" >> config/openstack.env
echo "OS_AUTH_TYPE=v3applicationcredential" >> config/openstack.env
echo "OS_APPLICATION_CREDENTIAL_ID=${APPLICATION_ID}" >> config/openstack.env
echo "OS_APPLICATION_CREDENTIAL_SECRET=${APPLICATION_SECRET}" >> config/openstack.env
